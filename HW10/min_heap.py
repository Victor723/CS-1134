...
class Heap:
    ...
    def find_less_than_or_equal_to(self,k):
        return self.find_less_helper(k,0)
    
    def find_less_helper(self,k,index):
        if index>self._size-1 or self._data[index]>k:
            return []

        elif self._data[index]==k:
            return [self._data[index]]

        else:
            l_index=self._left(index)
            left=self.find_less_helper(k,l_index)
            
            r_index=self._right(index)
            right=self.find_less_helper(k,r_index)
            
            return [self._data[index]]+left+right
...
if __name__ == '__main__':
    ...
    print(heap.find_less_than_or_equal_to(5))
