...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 19 20:47:04 2017
...
class BinarySearchTreeMap:
    ...
    def get_ith_smallest(self,i):
        if i > self.size:
            raise IndexError('List Index OutOfRange')
        def helper(curr,i):
            if curr.leftnum + 1==i:
                return curr.item.key
            elif i < curr.leftnum + 1:
                return helper(curr.left,i)
            elif i> curr.leftnum+1:
                return helper(curr.right,i-curr.leftnum-1)
        return helper(self.root,i)
    
def create_chain_bst(n):
    tree=BinarySearchTreeMap()
    for i in range(n):
        tree.subtree_insert(i+1)
    return tree
...
def find_min_abs_difference(bst):
    ...
    for i in bst.inorder():
        L.append(i.item.key)
    return min(abs(L[i+1]-L[i]) for i in range(len(L)-1))

def restore_bst(prefix_lst):
    tree = BinarySearchTreeMap()
    if len(prefix_lst)==0:
    	return tree
    NodeList = [BinarySearchTreeMap.Node(BinarySearchTreeMap.Item(i)) for i in prefix_lst]
    tree.root=NodeList[0]
    maxi=0
    for i in range(1,len(prefix_lst)):
        if prefix_lst[i] < prefix_lst[i-1]:
            NodeList[i].parent = NodeList[i-1]
            NodeList[i-1].left=NodeList[i]
        elif prefix_lst[i]>prefix_lst[maxi]:
            NodeList[maxi].right=NodeList[i]
            NodeList[i].parent=NodeList[maxi]
            maxi=i
        else:
            p=i
            while prefix_lst[i]>prefix_lst[p-1]:p-=1
            NodeList[p].right=NodeList[i]
            NodeList[i].parent=NodeList[p]
    return tree
...
def is_bst(bst):
    maxi=math.inf
    mini=-math.inf
    def helper(bst,root,maxi,mini):
        if root==None:
            return True
        if root.left.item.key>maxi or root.right.item.key<mini:
            return False
        else:
            return (helper(bst,root.left,root.data,mini) 
                   and helper(bst,root.right,maxi,root.data))
    return helper(bst,bst.root,maxi,mini)
