...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  6 16:00:19 2017
...
class InvertedFile:
    def __init__(self,file_name):
        ...
        for i in range(len(p)):
            ...
            for i in range(len(P)):
                if P[i][-1].isalpha():
                    self.L.append(P[i].lower())
                else:
                    self.L.append(P[i][:len(P[i])-1].lower())


        self.data=ChainingHashTableMap()
        for i in range(len(self.L)):
            if self.L[i] in self.data:
                self.data[self.L[i]].append(i)
            else:
                self.data[self.L[i]]=[i]
